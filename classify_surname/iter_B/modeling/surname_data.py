# import seq. 1) standard 2) 3rd party 3) local
import glob
import os
import re
import pprint
...
def save_df_surnames_as_pickle():
    ...
    test = df[train_cnt+1:]
    # save df per category
    df_train_per_category = dict()
    for i in range(len(df_categories)):
        df_train_per_category[i] = train.loc[train['index']==i]
    # save as pickle
    ...
    with open('data/pickles/df_train_per_category.pickle', 'wb') as handle:
        pickle.dump(df_train_per_category, handle, protocol=pickle.HIGHEST_PROTOCOL)
    # train test stat  
    t1 = train.groupby(['category']).count().drop(['normalized','freq','index'],axis=1)
    t2 = test.groupby(['category']).count().drop(['normalized','freq','index'],axis=1)
...
def load_df_surnames():
    ...
    df_train_per_category = pd.read_pickle('data/pickles/df_train_per_category.pickle')
    
    return df_train, df_test, df_train_per_category
